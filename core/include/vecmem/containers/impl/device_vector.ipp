...
namespace vecmem {
   ...
   : m_capacity( data.capacity() ), m_size( data.size_ptr() ),
     m_ptr( data.ptr() ) {
      ...
      VECMEM_DEBUG_MSG( 5, "Created vecmem::device_vector with capacity %lld "
                        " from pointer %p", m_capacity,
   ...
   device_vector< TYPE >::emplace_back( Args&&... args ) {
      ...
      atomic< size_type > asize( m_size );
      const size_type index = asize.fetch_add( 1 ) - 1;
   ...
   device_vector< TYPE >::pop_back() {
      ...
      atomic< size_type > asize( m_size );
      const size_type new_size = asize.fetch_sub( 1 );
